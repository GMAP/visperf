...
struct task_data {
    ...
    task_data(Mat _m, int _c) : m(_m), nframe(_c) {}
    task_data(task_data * task) : m(task->m), nframe(task->nframe) {}
    Mat m;
    int nframe;
};
...
class first_filter : public ff::ff_node_t<task_data> {
    ...
    int nframe;
public:
    first_filter(VideoCapture f) : fr(f), nframe(0) {}
    task_data* svc(task_data*){
        while(1){
            ...
            nframe++;
            task_data * task = new task_data(m,nframe);
            ff_send_out(task);
        }
    }
};
...
int main(int argc, char** argv) {
    ...
    ff::ff_OFarm<task_data> ofarm(move(workers));
    first_filter E(fr);
    ofarm.setEmitterF(E);
    last_filter C(fw);
